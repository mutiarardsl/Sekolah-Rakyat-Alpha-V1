 ...
 *    GET  /content/recommend  (fallback GET, POST adalah primary)
 *
 *  RAG — Tim 3 (5 endpoint)
 *    POST /content/recommend     (primary — Tim 3 komputasi RAG)
 *    POST /content/pretest/soal
 *    POST /content/pretest/submit  (body: siswa_id, mapel_id, elemen_id, materi_id?, sesi_pretest_id, answers — tanpa quiz_type)
 *    POST /content/insight           (Hero dashboard siswa)
 *    GET  /content/pretest/status  (status pretest per elemen/materi — hidrasi store lokal dari BE)
 *    GET  /content/quiz/history    (riwayat quiz per level + current_level + locked flag)
 *
 *  SUMMARY (1 endpoint — guru MonitoringSection)
 *    POST /summary/siswa/:id    (Tim 3 RAG)
 *
 *  EMOTION — Tim 1 (2 endpoint)
 *    POST /emotion/detect
 *    GET  /emotion/history
 *
 *  GAME — Tim 4 (4 endpoint — hirarki mapel→elemen→materi, deliver HTML)
 *    POST /game/generate
 *    GET  /game/list
 *    GET  /game/:game_id
 *    POST /game/selesai          (tracking penyelesaian game siswa)
 *
...
export const handlers = [
  ...
  ...(function v3BridgingHandlers() {
    ...
    return [
      ...
      http.post(url('/siswa/:siswaId/quiz'), async ({ request, params }) => {
        ...
        const j = await (async () => {
          const res = await fetch(`${BASE}/content/quiz/submit`, {
            method: 'POST',
            ...
            body: JSON.stringify({
              ...
              quiz_type: body.tipe === 'essay' ? 'essay' : 'mc',
              level: body.level,
              answers: body.jawaban,
              score: 0,
            }),
          });
          return res.json();
        ...
        // V3.1: generate hasil_quiz_id — opaque ID untuk CTA "Tanya Kak Nusa"
        ...
        return envelope({
          ...
          nilai:
            ...
            (body.tipe === 'mc'
              ? 80
              : 0),
          nilai_essay: j.essay_score ?? null,
          elemen_id: body.elemen_id,
          level: levelLc,
          agregasi: null,
          naik_level: false,
          kkm: j.kkm ?? 75,
          menunggu_agregasi: !!j.pending_aggregation,
          dicatat_at: j.recorded_at ?? new Date().toISOString(),
      ...
      http.post(url('/rag/rekomendasi'), async ({ request }) => {
        ...
        await d(250);
        const j = await (async () => {
          const res = await fetch(`${BASE}/content/recommend`, {
            method: 'POST',
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              levels: body.levels,
              completed_ids: body.sudah_selesai_ids,
              in_progress_ids: body.sedang_dipelajari_ids,
            }),
          });
          return res.json();
        })();
        return envelope(Array.isArray(j) ? j : []);
      }),
      ...
      http.post(url('/rag/insight'), async ({ request }) => {
        ...
        const body = await request.json();
        const j = await (async () => {
          const res = await fetch(`${BASE}/content/insight`, {
            method: 'POST',
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              nama: body.nama,
              streak: body.streak,
              total_topik: body.total_topik,
              total_poin_kuiz:
                body.total_poin_kuiz ?? body.total_poin_quiz,
              total_durasi: body.total_durasi,
            }),
          });
          return res.json();
        })();
      ...
      http.post(url('/pretest/soal'), async ({ request }) => {
        await d(200);
        const body = await request.json();
        const res = await fetch(`${BASE}/content/pretest/soal`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
      ...
      http.post(url('/pretest/submit'), async ({ request }) => {
        await d(450);
        const body = await request.json();
        const res = await fetch(`${BASE}/content/pretest/submit`, {
          method: 'POST',
          credentials: 'same-origin',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
      ...
      http.post(url('/konten/generate'), async ({ request }) => {
        await d(200);
        const body = await request.json();
        const backward = { ...body, guru_id: "g_bridge_mock" };
        if (backward.instruksi_revisi != null) {
          backward.revisi_guru = backward.instruksi_revisi;
          delete backward.instruksi_revisi;
        }
        const res = await fetch(`${BASE}/content/generate`, {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(backward),
        });
        const raw = await res.json();
        return envelope({
          ...raw,
          dibuat_at: raw.generated_at || raw.dibuat_at || new Date().toISOString(),
        });
      }),
      ...
      http.post(url('/konten/publish'), async ({ request }) => {
        ...
        const backward = {
          ...body,
          konten_list: (body.konten_list || []).map((c) => {
            const { disetujui, approved, ...rest } = c;
            return {
              ...rest,
              approved: disetujui ?? approved ?? true,
            };
          }),
        };
        const res = await fetch(`${BASE}/content/publish`, {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(backward),
        ...
        return envelope(
          {
            publish_id: raw.publish_id,
            kelas_ids: raw.kelas_ids,
            dipublish_at: raw.published_at || raw.dipublish_at,
          },
          null,
          201,
        );
      ...
      http.patch(url('/game/:gameId/penyelesaian'), async ({ request, params }) => {
        await d(120);
        const body = await request.json();
        const res = await fetch(`${BASE}/game/selesai`, {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        const raw = await res.json();
        return envelope({
          tercatat: raw.recorded ?? true,
          game_id: raw.game_id ?? params.gameId,
          siswa_id: raw.siswa_id ?? body.siswa_id,
          level: raw.level ?? body.level,
          selesai_at: raw.selesai_at ?? new Date().toISOString(),
        });
      }),
      ...
      http.post(url('/emosi/deteksi'), async ({ request }) => {
        await d(180);
        const body = await request.json();
        const res = await fetch(`${BASE}/emotion/detect`, {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            session_id: body.sesi_id,
          }),
        });
        const raw = await res.json();
        return envelope({
          emosi: raw.emosi ?? "tidak_terdeteksi",
          confidence: raw.confidence ?? 0,
          terdeteksi_at: raw.terdeteksi_at ?? new Date().toISOString(),
        });
      }),
      ...
      http.post(url('/mentor/pesan'), async ({ request }) => {
        ...
        chatSessions[sid].push({
          role: "ai",
          teks: j.reply ?? j.balasan ?? "",
          dikirim_at: iso(),
        });
        return envelope({
          balasan: j.reply ?? j.balasan ?? "",
          sesi_id: sid,
        });
      }),
      ...
      http.post(url('/notifikasi'), async ({ request }) => {
        await d(150);
        const body = await request.json();
        const res = await fetch(`${BASE}/guru/rekomendasi`, {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        ...
        const raw = await res.json();
        return envelope(
          { id: raw.id ?? `notif_${Date.now()}`, dibuat_at: raw.created_at },
          null,
          201,
        );
      }),

      http.get(url('/siswa/:siswaId/notifikasi'), async ({ request, params }) => {
        await d(140);
        const search = new URL(request.url).searchParams;
        const qs = new URLSearchParams({
          siswa_id: params.siswaId,
          dibaca: search.get('dibaca') || '',
          page: search.get('page') || '',
          limit: search.get('limit') || '',
        });
        const rows = await legacyJson(`/guru/rekomendasi?${qs}`);
        return envelope(Array.isArray(rows) ? rows : [], {
          page: 1,
          limit: 20,
  ...
  http.post(url('/auth/refresh'), async ({ request }) => {
    ...
    if (!account) {
      return HttpResponse.json({ message: 'Refresh token tidak valid atau sudah expired.' }, { status: 401 });
    }
    return HttpResponse.json({
  ...
  // POST /auth/aktivasi
  // Body: { user_id, password, mapel_ids } — atomik: password + 3 mapel pilihan sekaligus
  http.post(url('/auth/aktivasi'), async ({ request }) => {
    const { password, user_id, mapel_ids } = await request.json();
    await d(800);
    ...
    if (!Array.isArray(mapel_ids) || mapel_ids.length !== 3) {
      return HttpResponse.json({ message: 'Harus memilih tepat 3 mata pelajaran.' }, { status: 400 });
    }
    if (!password || password.length < 8) {
      return HttpResponse.json({ message: 'Password minimal 8 karakter.' }, { status: 400 });
    }
    siswa.status = 'Aktif';
  ...
  // ForceChangePasswordModal: old_password = '' (first-login, diizinkan)
  http.put(url('/auth/change-password'), async ({ request }) => {
    ...
    // First-login: old_password boleh kosong
    const isFirstLogin = acc?.is_first_login === true;
    if (!isFirstLogin && old_password && acc && acc.password !== old_password) {
      return HttpResponse.json({ message: 'Password lama salah.' }, { status: 401 });
    }
    if (!isFirstLogin && !old_password) {
  ...
  http.put(url('/auth/avatar'), async ({ request }) => {
    ...
    try {
      ...
      if (!file) return HttpResponse.json({ message: 'File tidak ditemukan.' }, { status: 400 });
      ...
      return HttpResponse.json({ avatar: dataUrl });
    } catch (err) {
      console.error('[MSW] PUT /auth/avatar error:', err);
      return HttpResponse.json({ message: 'Gagal memproses file.' }, { status: 400 });
    }
  }),
...
🔬 *Tantangan: Cari satu kasus nyata di mana penerapan ${topik} menghasilkan kesimpulan yang berbeda dari intuisi awal. Diskusikan temuanmu dengan Kak Nusa.*`,
    ...
    // Shape: { pertanyaan: Array<{ soal, rubrik, placeholder }> }
      ...
      pertanyaan: [
        {
          ...
          placeholder: `Tulis definisimu dan contoh konkretnya di sini...`,
        },
        {
          ...
          placeholder: `Jelaskan hubungan antar konsep secara terstruktur...`,
        },
        {
          ...
          placeholder: `Tuliskan analisis dan langkah-langkah penerapannya...`,
        },
        {
          ...
          placeholder: `Ceritakan tantanganmu dan strategi untuk mengatasinya...`,
        },
        {
          ...
          placeholder: `Tulis ringkasanmu seperti sedang menjelaskan kepada teman...`,
        },
      ],
    });
    ...
    const makeGame = (lv, topik, mapel) => {
      ...
      return {
        ...
        level: lv,
        status: 'ready',
        html_url: `https://game.sekolahrakyat.id/play/${deterministicId}?level=${lv}`,
      };
    };
  ...
  http.get(url('/content/progress/guru'), async ({ request }) => {
    ...
    const siswaProgress = kelasStudents.map(s => {
      ...
      return {
        ...
        level: seed % 3 === 0 ? 'Low' : seed % 3 === 1 ? 'Mid' : 'High',
  ...
  http.get(url('/admin/elemen'), async ({ request }) => {
    ...
    const mapelId = p.get('mapel_id');
    await d(250);
    // KURIKULUM_ELEMEN adalah sumber kebenaran untuk elemen per mapel
    // (ADMIN_MAPEL_LIST tidak menyimpan field elemen secara inline)
    if (mapelId) {
      const elemenRaw = KURIKULUM_ELEMEN[mapelId] || [];
      return HttpResponse.json(elemenRaw.map(e => ({ ...e, mapel_id: mapelId })));
    }
    // Tanpa filter: flatten semua elemen dari KURIKULUM_ELEMEN
    const all = Object.entries(KURIKULUM_ELEMEN).flatMap(([mId, elArr]) =>
      elArr.map(e => ({ ...e, mapel_id: mId }))
    );
    return HttpResponse.json(all);
  }),

  // POST /admin/elemen
  // Body: { mapel_id, label } — elemen hanya menyimpan id, mapel_id, label. fase/deskripsi_cp ada di mapel.
  http.post(url('/admin/elemen'), async ({ request }) => {
    const body = await request.json();
    await d(350);
    const mapel = store.mapel.find(m => m.id === body.mapel_id);
    if (!mapel) return HttpResponse.json({ message: 'Mapel tidak ditemukan.' }, { status: 404 });
    const newId = (body.label || '').toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '').slice(0, 20)
      + '_' + Date.now().toString().slice(-4);
    if ((mapel.elemen || []).some(e => e.id === newId || e.label === body.label)) {
      return HttpResponse.json({ message: 'Elemen sudah ada di mapel ini.' }, { status: 409 });
    }
    const newEl = { id: newId, mapel_id: body.mapel_id, label: body.label };
    mapel.elemen = [...(mapel.elemen || []), newEl];
    return HttpResponse.json(newEl, { status: 201 });
  }),

  // PUT /admin/elemen/:id
  // Body: { label } — hanya label yang bisa diubah di elemen
  http.put(url('/admin/elemen/:id'), async ({ request, params }) => {
    const body = await request.json();
    await d(300);
    let found = null;
    store.mapel = store.mapel.map(m => {
      const updated = (m.elemen || []).map(e => {
        // Hanya update label — id & mapel_id tidak bisa diubah; fase/deskripsi_cp ada di mapel
        if (e.id === params.id) { found = { ...e, label: body.label ?? e.label }; return found; }
        return e;
      });
      return { ...m, elemen: updated };
    });
    if (!found) return HttpResponse.json({ message: 'Elemen tidak ditemukan.' }, { status: 404 });
  ...
  http.delete(url('/admin/elemen/:id'), async ({ params }) => {
    ...
    store.mapel = store.mapel.map(m => ({
      ...m,
      elemen: (m.elemen || []).filter(e => e.id !== params.id),
    }));
    return HttpResponse.json({ deleted: true });
  }),
  ...
  // PUT /admin/kelas/:id/mapel/:mapel_id/guru
  // Body: { guru_id: string }
  http.put(url('/admin/kelas/:id/mapel/:mapel_id/guru'), async ({ request, params }) => {
    const body = await request.json();
    await d(300);
    const kelas = store.kelas.find(k => k.id === params.id);
    if (!kelas) return HttpResponse.json({ message: 'Kelas tidak ditemukan.' }, { status: 404 });
    kelas.mapelGuruMap = { ...(kelas.mapelGuruMap || {}), [params.mapel_id]: body.guru_id };
    return HttpResponse.json({ guru_id: body.guru_id });
  }),
